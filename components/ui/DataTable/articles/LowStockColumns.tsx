...
export const LowStockColumns: ColumnDef<IArticle>[] = [
    {
    ...
    {
        accessorKey: "packaging.name",
        header: "SUPPORT",
        cell: ({ row } : { row : any} ) => row.original.packaging.name || "PAS DE SUPPORT",
    },
...

    {
        ...
        cell: ({ row } : { row : any} ) => {
            ...
            const firstPlacement = row.getValue("suppliers")[0];  // Accéder au premier index
            
            // Vérifier si le premier placement existe et afficher son nom
            return firstPlacement ? (
            <div className="capitalize">{firstPlacement.name}</div>  // Exemple d'affichage du champ 'name'
            ) : (
            <div>Fournisseur non disponible</div>  // Si placements est vide ou non défini
            );
        }
        ,
    },
