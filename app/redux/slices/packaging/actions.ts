...
export const fetchPackagings= createAsyncThunk<IPackaging[]>(
    'packagings/fetchPackagings',
    async (_, { rejectWithValue }) => {

      try {
        const response = await getRequest<IPackaging[]>('packagings'); // Remplacez avec votre endpoint
        if (response.error) {
          return rejectWithValue(response.error);
        }
        return response.data as IPackaging[] ;
